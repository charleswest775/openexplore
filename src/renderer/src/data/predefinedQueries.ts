...
ORDER BY MAX(m.date) DESC`,
      },
      {
        ...
        icon: '📎',
        filePattern: 'sms.db',
        description: 'Files and media shared via messages',
        sql: `SELECT
  a.rowid AS id,
  datetime(m.date / 1000000000 + 978307200, 'unixepoch', 'localtime') AS sent_at,
  ...
  a.filename,
  a.mime_type,
  ROUND(a.total_bytes / 1024.0, 1) AS size_kb
FROM attachment a
...
ORDER BY p.Last, p.First`,
      },
      {
        ...
        sql: `SELECT
  p.ROWID AS id, p.First, p.Last, p.Organization,
  mv.value AS email, mv.label
FROM ABPerson p
JOIN ABMultiValue mv ON mv.record_id = p.ROWID AND mv.property = 4
    ...
    queries: [
      {
        ...
        label: 'Recent Photos',
        icon: '🖼️',
        filePattern: 'photos.sqlite',
        description: 'Photos and videos with GPS data',
        sql: `SELECT
  a.Z_PK AS id, a.ZFILENAME AS filename,
  datetime(a.ZDATECREATED + 978307200, 'unixepoch', 'localtime') AS taken_at,
  a.ZLATITUDE AS lat, a.ZLONGITUDE AS lon,
  a.ZDURATION AS video_duration_sec,
  a.ZHEIGHT, a.ZWIDTH
FROM ZASSET a
ORDER BY a.ZDATECREATED DESC LIMIT 500`,
      },
    ],
  },
];
